...
function console_docker_tools_autoinstall_check {
    console_warn "Checking if automated docker tools installation/update is possible"
    command -v lsb_release >/dev/null 2>&1 || {
      console_warn "command 'lsb_release' not found. Required for OS check";
      if hash apt-get 2>/dev/null
      then
        console_info "[apt-get found] Please install 'lsb_release' - 'sudo apt-get install lsb-release'"; cerr_miss_compose=1;
        return 0
      fi
      return 1
    }
    console_warn "Requires Ubuntu Linux or compatible"
    console_warn "Tested on Ubuntu $REQ_UBUNTU"
    distro=$(lsb_release -is)
    if [ "$distro" != "Ubuntu" ]
    then
      console_warn "Automated install supported only on Ubuntu"
      return 1
    fi
    console_info "OS: Ubuntu (Linux)"
    version=$(lsb_release -rs)
    console_info "Version: $version"

    if ver_lo $version $REQ_UBUNTU
    then
      console_warn "Your Ubuntu is older then $REQ_UBUNTU, we do not recommend auto-install, but..."
    fi
    if ver_hi $version $REQ_UBUNTU
    then
      console_warn "Docker tools auto-install not tested on your Ubuntu version"
    fi

    echo
    echo " --------------------------------------------------------"
    echo " |  if you want to proceed with auto installation       "
    echo " |                                                     "
    echo " |  ./$DOCKER_CONSOLE_SCRIPT_NAME update-docker-toolkit   "
    echo " |                                                      "
    echo " |  REQUIRES SUDO                                       "
    echo " --------------------------------------------------------"
    echo

    if [ "$version" != "$REQ_UBUNTU" ]
    then
      return 1
    fi
...
function console_docker_tools_autoremove {
  cerr_miss_compose=0
  cerr_miss_docker=0
  command -v docker >/dev/null 2>&1 || { cerr_miss_docker=1; }
  command -v docker-compose >/dev/null 2>&1 || { cerr_miss_compose=1; }

  if [ $cerr_miss_docker -eq 0 ]
  then
    running_containers=$(docker ps|grep -v 'CONTAINER ID'|wc -l)
    if [ $running_containers -gt 0 ]
    then
      console_error "Please stop all your containers first"
      return 1
    fi
  fi

  if [ $cerr_miss_docker -eq 0 -o $cerr_miss_compose -eq 0 ]
  then
    console_warn "Removing OS docker packages and etc configurations in ... 10 sec"
    console_warn "CTRL+C    - TO CANCEL"
    # sleep 10
    sudo apt-get -y remove docker docker-engine docker.io docker-compose
...
function console_docker_tools_autoinstall {
  console_info "Setting up docker package repository for Ubuntu"
  sudo apt-get install apt-transport-https ca-certificates curl software-properties-common
  curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add -
  sudo apt-key fingerprint 0EBFCD88
  sudo add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable"

  console_info "Updating packages repo db"
  sudo apt-get update
  console_info "Installing/updating python pip"
  sudo apt-get -y install python-pip
  sudo -H pip install --upgrade pip

  console_info "Installing tools"
  sudo apt-get -y install docker-ce
  sudo apt-get -y remove python-texttable # removing dist installed texttable which might be in conflict with version required by docker-compose , dist installed python modules can not be reinstalled by pip
  sudo -H pip install docker-compose

  console_info "Starting docker service"
  sudo systemctl start docker

  console_info "Adding user to docker group"
  user_name=$(whoami)
  ...
  then
    echo
    console_warn "${CC_RED}IMPORTANT${CC_NC} - You need to logout and login to gain access to new 'docker' user group"
    echo
  fi
}
...
function console_docker_tools_update_required {
    ...
    then
      ...
      if [ $output -gt 0 ]; then console_docker_tools_install_help; fi
      return 1
    fi
